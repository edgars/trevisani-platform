...
interface Props {
  ano: number; mes: number; slug: string;
  regime: string; veiculoId?: string;
}

export function DreMesesNav({ ano, mes, slug, regime, veiculoId }: Props) {
  function href(a: number, m: number) {
    return `/t/${slug}/financeiro/dre?ano=${a}&mes=${m}&regime=${regime}${veiculoId ? `&veiculoId=${veiculoId}` : ""}`;
  }
