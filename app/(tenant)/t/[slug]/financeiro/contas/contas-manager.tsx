...
import { Building2, CreditCard, Loader2, Pencil, Plus, Trash2, X } from "lucide-react";
...
import { criarContaAction, atualizarContaAction, excluirContaAction, toggleAtivoContaAction } from "./actions";
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 pt-2">
      <div className="space-y-1.5">
        ...
        <Input name="nome" defaultValue={conta?.nome} placeholder="Ex: Conta Principal Itaú" required disabled={isPending} />
      </div>
...
export function ContasManager({ slug, contas: initial }: { slug: string; contas: ContaData[] }) {
      ...
      {contas.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center gap-4 py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground">Nenhuma conta cadastrada. Crie a primeira conta bancária da loja.</p>
          </CardContent>
        </Card>
      ) : (
