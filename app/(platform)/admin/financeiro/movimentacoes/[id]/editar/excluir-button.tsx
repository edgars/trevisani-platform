...
export function ExcluirMovimentacaoButton({ movId }: { movId: string }) {
  ...
  function handleExcluir() {
    ...
    startTransition(async () => {
      const { error } = await excluirMovimentacaoAction(movId);
      if (error) toast.error(error);
      else { toast.success("Movimentação excluída."); router.push("/admin/financeiro/movimentacoes"); }
    });
  }
